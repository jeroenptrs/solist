...
Interactive `/role-switch` and `/role override` commands create session-only role bindings. They affect later prompts in the same running Solist process, but they are not written to the config file unless the user uses `/role set` or the headless `solist roles set` command. `/role` and `/roles` open TUI selectors for choosing the configured Solo agent choices for each role.

In orchestration mode, `solist_dispatch_role` is the preferred worker handoff path. It resolves the role binding, spawns one configured Solo agent by default, sends the role-framed assignment, and records the assignment comment on the Solo todo. If a request explicitly asks for multiple reviewers or all configured agents, the dispatch can use multiple mapped agents. Verification dispatch uses the same role-binding resolution path for the `verifier` role.
